...
export type PropHost = Pick<Renderer, 'bedCoversSide' | 'bedCoversVert' | 'bedFinialPost' | 'bedFootboardVert' | 'beginStructOutline' | 'breezeAt' | 'camera' | 'castBlob' | 'castContact' | 'castEdgeQuad' | 'castFloraShadow' | 'chestOpenness' | 'ctx' | 'drawChestBoss' | 'drawChestGilded' | 'drawChestIron' | 'drawChestMossy' | 'drawChestWood' | 'drawFlora' | 'drawGrowingFrame' | 'drawStallGood' | 'outlineOn' | 'paintGreatCloth' | 'paintShelfGood' | 'paintStandingHoop' | 'paintStreetCask' | 'queueGlow' | 'rubble' | 'signHasText' | 'sky' | 'sparkle' | 'stationClang' | 'stationHeat'>;
...
export interface PropFrame {
    ...
    stationBody(hw?: number, up?: number, down?: number): {
        x: number;
        y: number;
        w: number;
        h: number;
