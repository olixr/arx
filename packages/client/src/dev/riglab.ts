...
// MOTION DOCTRINE; recut for THE EQUINE LIMB pass: muscled
// gaskin/forearm-over-cannon legs, the hock point, the fetlock knot,
// and the ordered 4-beat lateral walk). Every saddle body (bay/grey/
// dun courser, the Hoargate garron, the Night Sabercat) across the
// eight facing bands at idle, walk (WATCH THE FOOTFALL ORDER — the
// walk rows must cycle LH, LF, RH, RF), and FULL GALLOP (the flight
// rig's aerial beat is the whole point — shoot the gallop rows
// animated, not just settled), plus the hurt-flash silhouette row, a
// grey-dapple seed-spread row, and BODY-RULER cells standing the
// player rig beside the courser and the sabercat so the saddle height
// reads at a glance. Each figure owns a live LegRig AND a live
// TailSim — the sheet plays the same physics the game does: gallop
// rows fly, stops settle the hair fall in one swing, the cat's rope
// hooks up through its last third.
// Levers:
