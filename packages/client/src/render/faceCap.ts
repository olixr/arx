 ...
 */
export const FACE_CELL_CAP_PX = 1024;
 ...
 */
export function faceCellScale(
  pwDev: number,
  phDev: number,
  capDim: number | undefined,
): number {
  if (capDim === undefined || capDim <= 0) return 1;
  const m = Math.max(pwDev, phDev);
  return m <= capDim ? 1 : capDim / m;
}
...
export function faceCellDims(
  pwDev: number,
  phDev: number,
  capDim: number | undefined,
): { cw: number; ch: number; k: number } {
  const k = faceCellScale(pwDev, phDev, capDim);
  if (k === 1) return { cw: pwDev, ch: phDev, k: 1 };
  return {
    cw: Math.max(1, Math.ceil(pwDev * k)),
    ch: Math.max(1, Math.ceil(phDev * k)),
    k,
  };
}
