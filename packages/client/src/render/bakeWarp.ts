 ...
 */
export function chunkNearBandEligible(depthScaleNearEdge: number): boolean {
  return depthScaleNearEdge >= 1;
}

export function chunkBakePxWarp(
  nearPx: number,
  farPx: number,
  depthScaleNearEdge: number,
): number {
  return chunkNearBandEligible(depthScaleNearEdge) ? nearPx : farPx;
}
 ...
 */
export function chunkResDeficit(bakedPx: number, wantedPx: number): boolean {
  return wantedPx > bakedPx;
}
...
export function inNearRing(
  ...
  return Math.abs(cx - camCx) <= radius && Math.abs(cy - camCy) <= radius;
}
 ...
 */
export function lodTierWarp(
  depthScale: number,
  curTier: number | undefined,
  hyst: number,
  minTier: number,
  maxTier: number,
): number {
  const c = Math.log2(depthScale) * 2;
  let tier = Math.round(c);
  if (curTier !== undefined) {
    if (tier <= curTier) {
      // Ratchet: a denser (or equal) sheet is already in hand — warp it
      // down, never re-bake to a sparser tier.
      tier = curTier;
    } else if (c - curTier < hyst) {
      // Nearer, but not by a full step — hold to avoid a jitter re-bake.
      tier = curTier;
    }
  }
