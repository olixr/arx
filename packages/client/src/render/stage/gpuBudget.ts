 ...
 */
export function admitUpload(msLeft: number, estMs: number, firstThisFrame: boolean): boolean {
  return firstThisFrame || estMs <= msLeft;
}

/** EMA step for the measured cost, in ms per MB. Fast-moving (0.3):
 *  the first real samples should quickly wash out the seed. */
export function nextUploadCost(emaMsPerMb: number, tookMs: number, bytes: number): number {
  if (bytes <= 0) return emaMsPerMb;
  const sample = tookMs / (bytes / MB);
  return emaMsPerMb + (sample - emaMsPerMb) * 0.3;
}
