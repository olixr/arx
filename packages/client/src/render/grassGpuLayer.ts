...
export class GrassGpuLayer {
  ...
  private renderBands(
    ...
    for (const band of bands) {
      ...
      // RIGID rise (uElev · scale · depthScale(rootRow)) so the slot bbox tracks
      // the lifted blades — computed once from the root row's divisor. 0 = flat.
      const elev = band.elev ?? 0;
      const liftPx = elev !== 0 ? (elev * f.scale) / proj(minBx, botY).wDiv : 0;
      let x0 = Infinity;
      let y0 = Infinity;
      let x1 = -Infinity;
      let y1 = -Infinity;
