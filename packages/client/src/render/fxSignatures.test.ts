 ...
 * so the reuse is a decision, not an accident. Passive signatures
 * (the burnished wall, the stubborn heart, the pounce perfected)
 * carry no cast wire and so no signature — natures, not workings.
 */
test('THE FANG FINDS ITS VOICE: every companion working speaks', () => {
  const TWO_STRATA = [
    'nip_and_dart',
    'plague_gnaw',
    'echo_shriek',
  ...
  for (const id of [...TWO_STRATA, ...THREE_STRATA]) {
    const sig = SIGNATURES[id];
    assert.ok(sig, `pet art '${id}' has no signature — a mute working`);
    const hooks = [sig.spawn, sig.ground, sig.air].filter(Boolean).length;
    assert.ok(hooks >= 2, `pet art '${id}' speaks on ${hooks} stratum — two is the floor`);
  }
  for (const id of THREE_STRATA) {
    const sig = SIGNATURES[id]!;
    assert.ok(
      sig.spawn && sig.ground && sig.air,
      `species signature '${id}' must speak all three strata`,
    );
  }
