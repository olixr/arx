...
export function waterfallItem(rend: PaintHost, 
  ...
  return {
    ...
    draw: () => {
      ...
      B.x = Math.round(B.x);
      B.y = Math.round(B.y);
      // The curtain spans two corners (ay north, by south); the whole
      // sheet/bands/threads ride yTop*/yBase* via the interpolators below.
...
export function fallRibbonItem(rend: PaintHost, 
  ...
  return {
    ...
    draw: () => {
      ...
      // The edge-on ribbon spans crest (r0) to landing (r1).
