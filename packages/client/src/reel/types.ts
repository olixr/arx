...
export type StageStep =
  /** A dev chat command (`/give`, `/time`…). Paced by the bucket. */
  | { k: 'cmd'; text: string }
  /**
   * Stand on this world point — and PROVE it. `/tp` refuses silently
   * when it cannot find a walkable tile within four of the mark, and a
   * shot that stages somewhere else performs perfectly in the wrong
   * place. This asks, checks, nudges, and finally gives up loudly.
   */
  | { k: 'at'; x: number; y: number }
  /** Equip the named item out of the pack (giving it first if needed). */
  | { k: 'equip'; item: string; stow?: boolean }
  /** Seat a technique in the R slot. */
  | { k: 'tech'; ability: string; slot?: 0 | 2 }
  /** Stand still and let the world settle (chunks, herds, grass). */
  | { k: 'wait'; ms: number }
  /** Walk to a world point before the camera rolls. */
  | { k: 'walk'; x: number; y: number; timeout?: number }
  /** Use the world at a tile — a portal mouth, a door, a station. */
  | { k: 'interact'; x: number; y: number };

/** The camera's job this instant. */
export type CamMove =
  /** Ride the player, held `lead` tiles toward their aim. */
  | { k: 'follow'; zoom?: number; lead?: number; ms?: number }
  /** Ride a foe — the crowned entrance shot. */
  | { k: 'followFoe'; zoom?: number; pick?: 'nearest' | 'biggest'; ms?: number }
  /** Hold a fixed world point. */
  | { k: 'hold'; x: number; y: number; zoom?: number; ms?: number }
  /** Glide from wherever the camera is to a world point over `ms`. */
  | { k: 'to'; x: number; y: number; zoom?: number; ms?: number; ease?: EaseName }
  /** Creep at a constant tiles/second — the slow trailer drift. */
  | { k: 'drift'; dx: number; dy: number; zoom?: number; ms?: number }
  /** Change only the zoom over `ms` (the push-in / pull-back). */
  | { k: 'zoom'; to: number; ms: number; ease?: EaseName };
...
export type Act =
  ...
  | { k: 'goto'; x: number; y: number; speed?: number; within?: number }
  /** Close on the picked foe — steer toward it every frame, facing it,
   *  and stop inside `within` tiles. The one act a fight cannot be
   *  written without: foes move, so a fixed walk vector is a guess that
   *  is wrong by the second beat. */
  | { k: 'closeOn'; pick?: 'nearest' | 'biggest'; within?: number; speed?: number }
  /** Back off the picked foe by the same logic — the circling beat. */
  | { k: 'backOff'; pick?: 'nearest' | 'biggest'; speed?: number }
  /** Look at a world point / a foe / an absolute angle. */
  | { k: 'face'; x: number; y: number }
  | { k: 'faceFoe'; pick?: 'nearest' | 'biggest' }
  | { k: 'aim'; rad: number }
  /** Release the director's grip on the eyes (aim follows the walk). */
  | { k: 'aimFree' }
  /** Hold a button for `ms`: the swing, the Art, the relic, the sigil. */
  | { k: 'press'; btn: Button; ms: number }
  /** A dev command mid-performance — the wave that charges in on cue. */
  | { k: 'cmd'; text: string }
  /** Use the world at a tile: harvest a row, open a door, work a bench.
   *  The verb every non-combat reel is actually about. */
  | { k: 'use'; x: number; y: number }
  /** Camera. */
  | { k: 'cam'; move: CamMove }
  /** Hand-held energy: 0 kills it, 1 is a combat shoulder-cam. */
  | { k: 'handheld'; amount: number }
  /** The renderer's own exclamation points. */
  | { k: 'shake'; amount: number }
  | { k: 'pulse'; amount?: number }
...
export interface Shot {
  ...
  seconds: number;
  /**
   * Capture and delivery frame rate. THIRTY IS THE HOUSE RATE and the
   * default: it is what every game trailer ships at, it halves both the
   * bitrate and the encode, and it doubles the frame budget so the
   * spikes that come with a wave of bodies stop costing a frame. Raise
   * it only for a shot that genuinely needs sixty.
   */
  fps?: number;
  hud: HudMode;
