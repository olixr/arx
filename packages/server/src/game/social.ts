...
export class SocialSystem {
  ...
  async snapshot(selfId: number, send: (msg: S2CMessage) => void): Promise<void> {
    const friends = (await this.accounts.loadFriends(selfId)).map((f) => {
    ...
    const requests = await this.accounts.loadFriendRequests(selfId);
    send({
      t: 'social',
      friends,
  ...
  async search(selfId: number, query: string, send: (msg: S2CMessage) => void): Promise<void> {
    const q = query.trim();
    if (q.length < 2) {
      send({ t: 'friendsearch', results: [] });
      return;
    }
    const results = (await this.accounts.searchCharacters(q, selfId, 10)).map((c) => ({
      name: c.name,
  ...
  async request(selfId: number, selfName: string, name: string, send: (msg: S2CMessage) => void): Promise<void> {
    ...
    if (await this.accounts.areFriends(selfId, target.id)) {
      return notice(`You are already friends with ${target.name}.`);
    }
    if ((await this.accounts.countFriends(selfId)) >= FRIEND_CAP) {
      return notice('Your friend ledger is full.');
    }
    if ((await this.accounts.countFriends(target.id)) >= FRIEND_CAP) {
      return notice(`${target.name}'s friend ledger is full.`);
    }
    if (await this.accounts.hasFriendRequest(selfId, target.id)) {
      return notice(`You have already asked ${target.name}.`);
    }
    if (await this.accounts.hasFriendRequest(target.id, selfId)) {
  ...
  async decline(selfId: number, selfName: string, name: string, send: (msg: S2CMessage) => void): Promise<void> {
    ...
    const target = await this.accounts.findCharacterByName(name.trim());
    if (!target) return notice('No one by that name.');
    if (await this.accounts.deleteFriendRequest(target.id, selfId)) {
    ...
    if (await this.accounts.deleteFriendRequest(selfId, target.id)) {
      notice(`Request to ${target.name} withdrawn.`);
      this.host.sendToCharacter(target.id, { t: 'friendevent', kind: 'declined', name: selfName });
    }
  }

  async remove(selfId: number, selfName: string, name: string, send: (msg: S2CMessage) => void): Promise<void> {
    const notice = (text: string) => send({ t: 'chat', channel: 'system', text });
    const target = await this.accounts.findCharacterByName(name.trim());
    if (!target) return notice('No one by that name.');
    if (!(await this.accounts.areFriends(selfId, target.id))) {
  ...
  async notifyOnline(characterId: number, name: string): Promise<void> {
    for (const f of await this.accounts.loadFriends(characterId)) {
  ...
  async pendingCount(characterId: number): Promise<number> {
    return (await this.accounts.loadFriendRequests(characterId)).incoming.length;
  }
}
