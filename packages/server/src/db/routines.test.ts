...
test('a changed shipped file flows into an untouched seed', async () => {
  ...
  const res = await seedRoutines(db, [edited, ...ALL.filter((r) => r.id !== 'hearth_hours')]);
...
test('THE DATABASE IS THE TRUTH: a tool edit survives every re-seed', async () => {
  ...
  const res = await seedRoutines(db, [shipped, ...ALL.filter((r) => r.id !== 'warden_rounds')]);
  assert.equal(res.kept, 1);
  const kept = (await exportRoutine(db, 'warden_rounds'))!;
  ...
  const again = await seedRoutines(db, [shipped, ...ALL.filter((r) => r.id !== 'warden_rounds')]);
...
test('import rejects unsound defs without writing', async () => {
  const db = await freshDb();
  await seedRoutines(db, ALL);
  const res = await importRoutine(db, { id: 'warden_rounds', base: { kind: 'wander', radius: 900 } });
  assert.ok(!res.ok);
  const kept = (await exportRoutine(db, 'warden_rounds'))!;
