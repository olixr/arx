...
test('a changed shipped file flows into an untouched seed', () => {
  const db = openDb(':memory:');
  seedRoutines(db, ALL);
  const smith = ALL.find((r) => r.id === 'smith_day')!;
  const edited: RoutineDef = { ...smith, slots: undefined };
  const res = seedRoutines(db, [edited, ...ALL.filter((r) => r.id !== 'smith_day')]);
  assert.equal(res.updated, 1);
  const loaded = loadRoutines(db);
  assert.equal(loaded.routines.find((r) => r.id === 'smith_day')!.slots, undefined);
});
...
test('THE DATABASE IS THE TRUTH: a tool edit survives every re-seed', () => {
  ...
  const tool = JSON.parse(JSON.stringify(ALL.find((r) => r.id === 'arch_patrol')!)) as RoutineDef;
  ...
  const shipped = JSON.parse(JSON.stringify(ALL.find((r) => r.id === 'arch_patrol')!)) as RoutineDef;
  shipped.slots![0]!.from = 22;
  const res = seedRoutines(db, [shipped, ...ALL.filter((r) => r.id !== 'arch_patrol')]);
  assert.equal(res.kept, 1);
  const kept = exportRoutine(db, 'arch_patrol')!;
  const keptTask = kept.slots![0]!.task;
  assert.ok(keptTask.kind === 'path');
  assert.equal(keptTask.waypoints[1]!.x, 4.0, 'the tool edit stands');
  assert.equal(kept.slots![0]!.from, 21, 'the shipped change never landed');

  // And the divergence is not re-weighed on the next identical seed.
  const again = seedRoutines(db, [shipped, ...ALL.filter((r) => r.id !== 'arch_patrol')]);
...
test('a retired shipped file prunes only its untouched seed', () => {
  ...
  const tool = JSON.parse(JSON.stringify(ALL.find((r) => r.id === 'road_wanderer')!)) as RoutineDef;
  tool.base = { kind: 'post' };
  assert.ok(importRoutine(db, tool).ok);
  const remaining = ALL.filter((r) => r.id !== 'road_wanderer' && r.id !== 'plaza_idler');
  const res = seedRoutines(db, remaining);
  assert.equal(res.removed, 1, 'the pure seed goes');
  const loaded = loadRoutines(db);
  assert.ok(loaded.routines.some((r) => r.id === 'road_wanderer'), 'the tool-owned row stays');
  assert.ok(!loaded.routines.some((r) => r.id === 'plaza_idler'), 'the retired seed is gone');
});

test('import rejects unsound defs without writing', () => {
  const db = openDb(':memory:');
  seedRoutines(db, ALL);
  const res = importRoutine(db, { id: 'arch_patrol', base: { kind: 'wander', radius: 900 } });
  assert.ok(!res.ok);
  const kept = exportRoutine(db, 'arch_patrol')!;
