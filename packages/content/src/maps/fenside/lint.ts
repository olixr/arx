/**
 * THE FEN WAIST (contested lands, band 7) — THE FLOODS (CURATION LAW 6).
 *
 * Pure reads over the built ZoneDef (plus the scenes' registry);
 * fenside.test asserts each returns []. The Ashlamp's lints COPIED
 * (themselves the Dawnmead lints copied for a patch on worldgen; no
 * shared lib until the Sett is the third consumer, R1), plus the one
 * law the bar has that the scar does not: gapOpen, the warden's gap.
 *
 ...
 *  - emberBedsOffAsh:    every EmberBed sits IN the ash, never on it
 *                        (fix pass 1's reading of K1: the bed's own
 *                        cell carries no ash detail and at least two of
 *                        its cardinal neighbours do; none stands here,
 *                        the read is kept whole).
 ...
 *                        (BAR_GAP: the north post and the stepped tooth).
 ...
 *  - gapOpen:            the warden's gap is the road's own bed, the
 *                        only passable cell of its column between the
 *                        teeth, and walked to from both ends (R2).
 */
import {
  Detail,
  TILE_DEFS,
...
import type { FenRegistry, Pt } from './ctx.js';
 ...
 */
export function occlusionViolations(z: ZoneDef, registry?: FenRegistry): string[] {
...
export function boxOverlaps(registry: FenRegistry): string[] {
  const out: string[] = [];
...
/**
 * THE WARDEN'S GAP (R2): the road at the bar is narrowed to one pair
 * of boots and never shut. The gap cell stays the field's own bed
 * (TILE_SKIP, never authored), it is the ONLY passable cell of its
 * column between the outer teeth (every other cell there is a post
 * or a tooth, solid), and it is reached on foot from the bed on both
 * sides of the bar: `west` and `east` are bed cells the flood starts
 * from (the interior only; the field's ground passes).
 */
export function gapOpen(
  z: ZoneDef,
  gap: Pt,
  teeth: ReadonlyArray<Pt>,
  west: Pt,
  east: Pt,
): string[] {
  const out: string[] = [];
  const [gx, gy] = gap;
  if (groundAt(z, gx, gy) !== TILE_SKIP) out.push(`the gap (${gx},${gy}) is authored; it must stay the road's bed`);
  const ys = teeth.filter(([x]) => x === gx).map(([, y]) => y);
  if (ys.length < 2) out.push(`the teeth do not flank the gap's column x ${gx}`);
  const y0 = Math.min(...ys);
  const y1 = Math.max(...ys);
  for (let y = y0; y <= y1; y++) {
    if (y === gy) continue;
    const t = groundAt(z, gx, y);
    if (t === undefined || t === TILE_SKIP || !TILE_DEFS[t as Tile].solid) {
      out.push(`(${gx},${y}) beside the gap is passable; the bar has a second gap`);
    }
  }
  const reaches = (from: Pt): boolean => {
  ...
  if (!reaches(west)) out.push(`the gap (${gx},${gy}) is not reached from the west bed at (${west[0]},${west[1]})`);
  if (!reaches(east)) out.push(`the gap (${gx},${gy}) is not reached from the east bed at (${east[0]},${east[1]})`);
  return out;
}
