...
export function buildBramblewick(): ZoneDef {
  ...
  // each keeping the hours of a routine (content/routines; routine
  // coordinates are offsets from the post). Guards flank the south
  // arch run and pace it after dark; the captain keeps the north arch
  // — the untargetable dialogue anchor holds absolutely still.
  b.actor('bramblewick_gate_guard', 46.3, 55.5, Math.PI / 2, 'arch_patrol');
  b.actor('bramblewick_gate_guard', 50.3, 55.5, Math.PI / 2, 'arch_patrol');
  b.actor('captain_alda', 46.4, 41.5, Math.PI / 2);
  // Hetty rounds her fenced plots by day; Tobbin keeps shop hours at
  // the counter; Maren ambles the plaza between bench sits; Brannock
  // works anvil and furnace, lunches at the campfire, sleeps at the
  // forge.
  b.actor('farmer_hetty', 17.5, 27.5, 0, 'farmhand_day');
