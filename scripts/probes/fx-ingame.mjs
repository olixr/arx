// PARTICLES v6 in-world probe: casts a library effect through renderer.castEffect on a rig lane (ORIGIN=, USER_=, PASS_=).
...
const delays = (process.argv[3] ?? '250,900,2200').split(',').map(Number);
...
await page.evaluate((id) => {
  ...
  r.castEffect(id, own.x + 0.9, own.y + 0.35, { dir: 0, radius: 1 });
}, fx);
